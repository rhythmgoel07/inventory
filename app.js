...
async function handleExcelExport() {
  ...
  try {
    ...
    const headers = ["SKU", "Image", "Item Name", "Category", "Material", "Supplier",
      "Warehouse/Location", "Cost Price", "Selling Price", "Margin %", "Stock Qty",
      "Reorder Level", "Stock Value", "Stock Status"];
    ws.addRow(headers);
    ...
    for (const it of ITEMS) {
      ...
      if (it.image && it.image.startsWith("data:image")) {
        try {
          const ext = it.image.substring(11, it.image.indexOf(";")); // "jpeg" or "png"
          const imgId = wb.addImage({ base64: it.image, extension: ext === "jpeg" ? "jpeg" : "png" });
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = `inventory_export_${todayStamp()}.xlsx`; a.click();
    URL.revokeObjectURL(url);
    const withPhotos = ITEMS.filter((it) => it.image).length;
